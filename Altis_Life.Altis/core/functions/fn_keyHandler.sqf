...
switch (_code) do
{
	...
	case 35:
	{
		if(_shift && !_ctrlKey && currentWeapon player != "") then {
			...
			player action ["Waffe wechseln", player, player, 100];
		...
		{
			{
				...
				if(_val > 0 ) then
				{
					if( _str == "Bohrer" || _str == "bohrer" ) then
					{
						[] spawn life_fnc_bohrerUse;
					};
				};
			} foreach life_inv_items;
		}
		...
		if(!_alt && !_ctrlKey) then {
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				if(_veh in life_vehicles && player distance _veh < 8) then {
					...
					if(_door == 0) exitWith {hint "In deiner nähe befindet sich keine Tür!"};
					_locked = _veh getVariable [format["bis_disabled_Door_%1",_door],0];
					if(_locked == 0) then {
						_veh setVariable[format["bis_disabled_Door_%1",_door],1,true];
						_veh animate [format["door_%1_rot",_door],0];
						systemChat "Tür abgeschlossen.";
					} else {
						_veh setVariable[format["bis_disabled_Door_%1",_door],0,true];
						_veh animate [format["door_%1_rot",_door],1];
						systemChat "Tür aufgeschlossen.";
					};
				};
			} else {
				_locked = locked _veh;
