...
if(life_action_inUse) exitWith {hint "Du führst bereits eine Aktion aus, warte bitte bis diese vorbei ist."};
_search = nearestObjects[getPos air_sp, ["Air"],5];
if(count _search == 0) exitWith {hint "Es befindet sich kein Helikopter am Helipad!"};
if(life_cash < 1000) exitWith {hint "Du brauchst $1.000 um dein Helikopter zu warten"};
...
if(!alive (_search select 0) || (_search select 0) distance air_sp > 10) exitWith {life_action_inUse = false; hint "Das Fahrzeug ist zerstört oder es befindet sich nicht mehr Helipad!"};
...
5 cutText ["","PLAIN"];
titleText ["Dein Helikopter wurde repariert und aufgetankt.","PLAIN"];
