...
if(life_cash < _basePrice) exitWith {hint format["Du hast nicht genug Geld, dir fehlen $%1.",[_basePrice - life_cash] call life_fnc_numberText];};
...
if(_spawnPoint == "") exitWith {hint "Ein Fahrzeug blockiert den Spawnpunkt";};
life_cash = life_cash - _basePrice;
hint format["Du hast einen %1 gekauft für $%2",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_basePrice] call life_fnc_numberText];
