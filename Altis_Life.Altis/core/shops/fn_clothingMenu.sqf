...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","kart"] && playerSide != civilian) exitWith {hint "Du musst ein Zivilist sein um diesen Händler zu benutzen!"; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint "Du hast kein Rebellen Training!"; closeDialog 0;};
...
{
	if(!(missionNamespace getVariable (_var select 0))) exitWith {hint format["Du brauchst %1 um von diesem Händler kaufen zu können!",[_var select 0] call life_fnc_varToStr]; closeDialog 0;};
};
