...
_unit spawn
{
	...
	waitUntil {_Timer ctrlSetText format["wiederbeleben in: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString]; 
	round(_maxTime - time) <= 0 OR isNull _this};
	_RespawnBtn ctrlEnable true;
	_Timer ctrlSetText "Du kannst dich jetzt Wiederbeleben";
};
...
if(side _killer == west && playerSide != west) then {
	...
	if(!life_use_atm && {life_cash > 0}) then {
		[format["Der Bankräuber wurde getötet, das gestohlene Geld in Wert von $%1 wurde wieder in die Zentralbank gelagert",[life_cash] call life_fnc_numberText],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		life_cash = 0;
	};
};
