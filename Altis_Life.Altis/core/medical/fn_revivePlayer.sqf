...
if(_target getVariable ["Reviving",ObjNull] == player) exitWith {hint "Ein anderer Arzt wiederbelebt schon diese Person";};
...
if(_target getVariable["Revive",FALSE]) exitWith {hint "Die Person ist neugestartet oder wurde schon wiederbelebt."};
if((player getVariable["restrained",false])) exitWith {life_action_inUse = false;};
if(!isNil "_badDistance") exitWith {titleText["Du bist zu weit entfernd vom Zielgebiet.","PLAIN"]; life_action_inUse = false;};
...
titleText[format["%1 hat dich wiederbelebt, es wurden $%2 von Ihrem Konto abgebucht für diesen Service.",_targetName,[(call life_revive_fee)] call life_fnc_numberText],"PLAIN"];
