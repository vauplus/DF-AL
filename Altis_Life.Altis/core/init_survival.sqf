...
[] spawn
{
	...
	while{true} do
	{
		waitUntil {sleep 1; backpack player != ""};
		_bp = backpack player;
		_cfg = getNumber(configFile >> "CfgVehicles" >> (backpack player) >> "maximumload");
		_load = round(_cfg / 8);
		if (backpack player == "B_AssaultPack_cbr") then { _load = 20; };
		if (backpack player == "B_FieldPack_ocamo") then { _load = 30; };
		if (backpack player == "B_Bergen_sgg") then { _load = 30; };
		if (backpack player == "B_TacticalPack_oli") then { _load = 40; };
		if (backpack player == "B_Kitbag_mcamo") then { _load = 60; };
		if (backpack player == "B_Kitbag_cbr") then { _load = 60; };
